...
local function OnXPEvent()
	if restBar:IsShown() then
		if GetWatchedFactionInfo() then
			...
			local p="player";x=UnitXP(p);m=UnitXPMax(p);r=GetXPExhaustion();
			text:SetText(max - value .. " tnl - "..(math.floor(20*r/m+0.5)).." bubbles")
		end
	end
end
...
local function OnCreate(parent)
	...
	xpBar:SetScript('OnLeave', function() text:Show() end)
